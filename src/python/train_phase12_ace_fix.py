...
Phase 10 (150M) をベースに、修正済みC++ evaluator (ACE=最強) で再学習。

修正内容:
- ACE=0(最弱) → ACE=14(最強) に修正
- FLUSHキッカー情報を正しく保存
- フォール判定の24.8%誤判定を解消
...
SAVE_DIR = 'models/phase12'
CHECKPOINT_PREFIX = 'p12_ace_fix'
BASE_MODEL = 'models/phase10_gcp/p10_fl_stay_150000000.zip'


class Phase12SelfPlayEnv(gym.Env):
    """Phase 12 Self-Play環境: ACE修正版"""
    ...
    def __init__(self, env_id: int = 0):
        ...
        # FL報酬パラメータ (Phase 10と同じ)
        self.fl_entry_bonus = 50.0
        self.fl_stay_bonus = 100.0
...
def make_env(env_id):
    def _init():
        env = Phase12SelfPlayEnv(env_id=env_id)
        env = ActionMasker(env, lambda e: e.action_masks())
        return env
    return _init


class Phase12Callback(BaseCallback):
    """Phase 12学習コールバック"""

    def __init__(self, model_save_path, notifier=None, resume_step=0, total_target=200000000, verbose=0):
    ...
    def _on_training_start(self):
        ...
        if self.notifier:
            self.notifier.send_start({
                'phase': 'Phase 12 ACE Fix',
                'timesteps': self.total_target,
                'base_model': 'Phase 10 (150M) + ACE修正evaluator',
            })
    ...
    def _on_rollout_end(self):
        ...
        if steps >= self.last_checkpoint_step + checkpoint_interval:
            ...
            path = f"{self.model_save_path}/{CHECKPOINT_PREFIX}_{self.last_checkpoint_step}.zip"
            self.model.save(path)
            print(f"  Saved checkpoint: {path}")


def main():
    parser = argparse.ArgumentParser(description='Phase 12 ACE Fix Training')
    parser.add_argument('--steps', type=int, default=200000000,
                        help='Total training steps')
    parser.add_argument('--num-envs', type=int, default=None,
                        help='Number of parallel environments')
    parser.add_argument('--test-mode', action='store_true',
                        help='Test mode (short run)')
    ...
    # Phase 12チェックポイントを探す（再開用）
    checkpoints = glob.glob(f'{SAVE_DIR}/{CHECKPOINT_PREFIX}_*.zip')
    def _extract_step(path):
        m = re.search(rf'{CHECKPOINT_PREFIX}_(\d+)\.zip', path)
    ...
    if checkpoints:
        ...
        print(f"Loading Phase 10 model: {BASE_MODEL}")
        print("*** ACE修正evaluatorで再学習開始 ***")
        model = MaskablePPO.load(BASE_MODEL, env=env)
        resume_step = 0
        remaining_steps = args.steps

    # ハイパーパラメータ (Phase 10と同じ)
    model.learning_rate = 0.0001
    model.ent_coef = 0.02

    notifier = TrainingNotifier(
        discord_webhook=DISCORD_WEBHOOK_URL,
        project_name="OFC AI Phase 12"
    )
    ...
    if resume_step > 0:
        print(f"\nResuming Phase 12 training from {resume_step:,} steps...")
    else:
        print(f"\nStarting Phase 12 training for {args.steps:,} steps...")
    print(f"Target: Correct ACE ranking, improved foul accuracy\n")
    ...
    final_path = f'{SAVE_DIR}/{CHECKPOINT_PREFIX}_{args.steps}.zip'
