"""
OFC Pineapple AI - Phase 7 AWS Distillation Training
...
from s3_utils import S3Manager, init_s3
...
class SuperhumanOpponentManager:
    ...
    def get_opponent_action(self, agent_name, wrapper, current_model):
        ...
        if p < 0.8:
            ...
            # 現在のモデルと同一構造の別インスタンスを使うか、
            # 一時的に重みを差し替えて推論（学習中なので勾配計算に注意）
            with torch.no_grad():
    ...
    def _predict_ppo(self, model, wrapper, agent):
        ...
        # Use deterministic=True for opponents to improve stability (especially at start)
...
class Phase7Callback(BaseCallback):
    def __init__(self, save_path, notifier, s3_manager, opponent_manager, update_freq=200_000, report_freq=100_000):
        super().__init__()
    ...
    def _on_step(self):
        ...
        # 定期レポート
        if self.n_calls - self.last_report >= self.report_freq:
            self.last_report = self.n_calls
            if self.notifier:
                metrics = {
                    ...
                    'fps': self.n_calls / (time.time() - self.start_time)
                }
                self.notifier.send_progress(self.n_calls, 20_000_000, metrics)

        # チェックポイント保存 & 対戦相手プール更新
        if self.n_calls - self.last_update >= self.update_freq:
            ...
            # 容量確保のためのクリーンアップ (最新3つと1Mごとの節目を残す)
...
def train_aws_phase7():
    ...
    # S3 / Notifier
    s3 = init_s3()
    notifier = TrainingNotifier(discord_webhook=DISCORD_WEBHOOK_URL, project_name="Phase 7: AWS Distillation")
    ...
    if checkpoints:
        ...
        model = MaskablePPO(
            ...
            n_steps=2048,
            batch_size=128,
            tensorboard_log="./logs/phase7_distill/"
        )
        # If no checkpoint, try to load base model weights
    ...
    callback = Phase7Callback("models/p7_mcts", notifier, s3, opponent_manager)
    ...
    print(f"Starting Distillation Training (Goal: {total_steps:,} steps)")
    model.learn(
